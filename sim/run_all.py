...
def export_dsp_coeffs(inc):
    """Fixed filters for the dosimeter, hear-through and seal monitor (generated here so they match the sim)."""
    import seal_ref
    from noise import a_weighting_sos
    ...
    sos_seal = seal_ref.band_sos().reshape(-1, 6)
    ...
    seal = (f"#define SEAL_NB {len(seal_ref.BANDS)}u          /* octave bands "
            + ", ".join(f"{b:g}" for b in seal_ref.BANDS) + " Hz, 2 biquads each */\n"
            f"#define SEAL_GATE_DB {cf(seal_ref.GATE_DB)}\n#define SEAL_MIN_BAND_DB {cf(seal_ref.MIN_BAND_DB)}\n"
            f"#define SEAL_FLAG_DB {cf(seal_ref.FLAG_DB)}\n#define SEAL_HYST_DB {cf(seal_ref.HYST_DB)}\n"
            f"#define SEAL_AVG_S {cf(seal_ref.AVG_S)}\n#define SEAL_HOLD_S {seal_ref.HOLD_S}u\n")
